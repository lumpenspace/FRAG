...
It fetches the closest n embeddings to a given input and, if they are contiguous,
returns them as a single text block.
"""
...
class EmbeddingsReader(BaseModel):
    ...
    def get_similar(self, text: str, n_results: int = 3, **kwargs) -> List[Chunk]:
        """
        ...
        try:
            db_results = self.store.find_similar(
                text=text, n_results=n_results or self.n_results
            )

            return DBChunk.from_db_results(**db_results) if db_results else []
